...
var access_token = 'BQDhQWm0ScflWglI63WFFghKtGBMfWgCml2URS4vbvRGHMRFSy8ZILy6ZerPuK_MJk4p7_VIQ0IWbmT65aMk91a_tS0lP4Q7ATe40PjUt3VQPwQshdJsItuzDZb6qyHzD5_jpaVcZRAOCxB7evxPLCPZIq4UXW4',
...
function analysis (analysis_id){
    spotifyApi.getAudioAnalysisForTrack(analysis_id, function (err, data){
...
function features (features_id){
    spotifyApi.getAudioFeaturesForTrack(features_id,function (err, data){
        if (err) console.error(err);
        else console.log(data);
/*      document.getElementById("text").innerHTML = `
      ${data.map(function(data){
        ...
        return `
          ...
          Dancability ${data.danceability}<br>
          Energy ${data.energy}<br>
          Instrumentalness ${data.instrumentalness}<br>
          Key ${data.key}<br>
          Liveness ${data.liveness}<br>
          Mode ${data.mode}<br>
          Speechiness ${data.speechiness}<br>
          Tempo ${data.tempo}<br>
...
spotifyApi.getPlaylistTracks('4KbVm88FtGdO3pGRwkCWko',{limit:12},function(err,data){
    ...
    data.items.forEach(item => {
        song =  `
        <iframe src="https://open.spotify.com/embed/track/${item.track.id}" width="300" height="300" frameborder="0" allowtransparency="true" allow="encrypted-media"></iframe>
        `
        featurebutton = `
        <div class="btn-group">
       ...
       $("#buttons").append(featurebutton)     
        $("#tracks").append(song)
       // feature += ',' +(item.track.id);
        //analysis(item.track.id);     
    });
    //features(feature);
...
$(document).ready(function(){
  ...
  function Search(search){
      ...
      $.get("https://api.spotify.com/v1/search?q="+search+"&type="+type+"&limit=12&access_token="+access_token,function(data){
          console.log(data)
          $("#songs").empty()
          $("#album").empty()
          $("#playlist").empty()
          ...
          data.tracks.items.forEach(item => {
              song =  `
              <iframe src="https://open.spotify.com/embed/track/${item.id}" width="300" height="300" frameborder="0" allowtransparency="true" allow="encrypted-media"></iframe>
              `               
         ...
         data.albums.items.forEach(item => {
          album =  `
          <iframe src="https://open.spotify.com/embed/album/${item.id}" width="300" height="500" frameborder="0" allowtransparency="true" allow="encrypted-media"></iframe>
          `               
          $("#album").append(album)
     });
  }
...
function lyrics (song) {
  $.get("https://api.genius.com/search?q="+song+"&access_token="+genius_access_token,function(data){
    console.log(data) 
    
    glyric =`
    <a href="${data.response.hits[0].result.url}">Genius Lyrics</a>
    `
    $("#text").html(glyric)
  })
};
